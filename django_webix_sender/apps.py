        ...
        for send_method in self.CONF['send_methods']:
            ...
            if 'function' not in send_method:
                if send_method['method'] == 'email':
                    send_method['function'] = 'django_webix_sender.send_methods.email.send'
                elif send_method['method'] == 'skebby':
                    send_method['function'] = 'django_webix_sender.send_methods.skebby.send'
                elif send_method['method'] == 'telegram':
                    send_method['function'] = 'django_webix_sender.send_methods.telegram.send'
                elif send_method['method'] == 'storage':
                    send_method['function'] = 'django_webix_sender.send_methods.storage.send'
                else:
            ...
            if 'recipients_clean' not in send_method:
                if send_method['method'] == 'email':
                    send_method['recipients_clean'] = 'django_webix_sender.send_methods.email.recipients_clean'
                elif send_method['method'] == 'skebby':
                    send_method['recipients_clean'] = 'django_webix_sender.send_methods.skebby.recipients_clean'
                elif send_method['method'] == 'telegram':
                    send_method['recipients_clean'] = 'django_webix_sender.send_methods.telegram.recipients_clean'
                elif send_method['method'] == 'storage':
                    send_method['recipients_clean'] = 'django_webix_sender.send_methods.storage.recipients_clean'
                else:
                    raise ImproperlyConfigured(_("`recipients_clean` is not configured in send method"))
            # Check presend check
            if 'presend_check' not in send_method:
                if send_method['method'] == 'email':
                    send_method['presend_check'] = 'django_webix_sender.send_methods.email.presend_check'
                elif send_method['method'] == 'skebby':
                    send_method['presend_check'] = 'django_webix_sender.send_methods.skebby.presend_check'
                elif send_method['method'] == 'telegram':
                    send_method['presend_check'] = 'django_webix_sender.send_methods.telegram.presend_check'
                elif send_method['method'] == 'storage':
                    send_method['presend_check'] = 'django_webix_sender.send_methods.storage.presend_check'
                else:
            ...
            if 'attachments_format' not in send_method:
                if send_method['method'] == 'email':
                    send_method['attachments_format'] = 'django_webix_sender.send_methods.email.attachments_format'
                elif send_method['method'] == 'skebby':
                    send_method['attachments_format'] = 'django_webix_sender.send_methods.skebby.attachments_format'
                elif send_method['method'] == 'telegram':
                    send_method['attachments_format'] = 'django_webix_sender.send_methods.telegram.attachments_format'
                elif send_method['method'] == 'storage':
                    send_method['attachments_format'] = 'django_webix_sender.send_methods.storage.attachments_format'
                else:
        ...
        for initial_send_method in self.CONF['initial_send_methods']:
            ...
            if 'function' not in initial_send_method:
                if initial_send_method['method'] == 'email':
                    initial_send_method['function'] = 'django_webix_sender.send_methods.email.send'
                elif initial_send_method['method'] == 'skebby':
                    initial_send_method['function'] = 'django_webix_sender.send_methods.skebby.send'
                elif initial_send_method['method'] == 'telegram':
                    initial_send_method['function'] = 'django_webix_sender.send_methods.telegram.send'
                elif initial_send_method['method'] == 'storage':
                    initial_send_method['function'] = 'django_webix_sender.send_methods.storage.send'
                else:
                    raise ImproperlyConfigured(_("`function` is not configured in send method"))
